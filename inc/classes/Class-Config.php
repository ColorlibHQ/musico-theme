...
final class Musico {
	...
	private function setup() {
		...
		$enqueu          = new musico_Enqueue();
		$enqueu->scripts = $this->enqueue();
		$enqueu->musico_scripts_enqueue_init();
	...
	public function support() {
		// content width
		$GLOBALS['content_width'] = apply_filters( 'musico_content_width', 751 );


		// text domain for translation.
		load_theme_textdomain( 'musico', MUSICO_DIR_PATH . '/languages' );
		...
		add_theme_support( 'post-thumbnails', array( 'post', 'musico' ) );

		// Site logo size
		add_image_size( 'musico_site_logo_160x36', 160, 36, true );

		// Music man thumb sizes
		add_image_size( 'musico_music_man_thumb_148x148', 148, 148, true );

		// About thumb sizes
		add_image_size( 'musico_about_thumb_490x538', 490, 538, true );
		add_image_size( 'musico_about_signature_img_143x60', 143, 60, true );
		
		// Youtube videos thumbnail size
		add_image_size( 'musico_youtube_video_thumb_477x440', 477, 440, true );
		
		// Latest post thumbnail size
		add_image_size( 'musico_widget_post_thumb', 80, 80, true );

		// Single blog post image size
		add_image_size( 'musico_single_blog_750x375', 750, 375, true );
		add_image_size( 'musico_np_thumb', 60, 60, true );
		...
		register_nav_menus( array(
			'primary-menu' => esc_html__( 'Primary Menu', 'musico' ),
			'footer-menu'  => esc_html__( 'Footer Menu', 'musico' ),
		) );
	...
	private function enqueue() {
		...
		$cssPath = MUSICO_DIR_CSS_URI;
		$jsPath  = MUSICO_DIR_JS_URI;
		...
		$scripts = array(
			'style'   => array(
				...
				array(
					'handler'    => 'audioplayer-css',
					'file'       => $cssPath . 'audioplayer.css',
					'dependency' => array(),
					'version'    => '1.0',
				),
				array(
				...
				array(
					...
					'dependency' => array(),
					'version'    => $this->musico_version,
				),
				array(
					'handler' => 'musico-style',
			...
			'scripts' => array(
				...
				array(
					'handler'    => 'audioplayer-js',
					'file'       => $jsPath . 'audioplayer.js',
					'dependency' => array( 'jquery' ),
					'version'    => '1.0',
					'in_footer'  => true,
				),
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'musico' ) ) {
			
			$font_families = array(
				'Josefin+Sans:300,300i,400,400i,600,700',
				'Muli:300,300i,400,400i,600,600i,700,700i,900',
			);
	...
	private function customizer_init(){
		...
		// Instantiate musico theme customizer
		$musico_theme_customizer = new musico_theme_customizer();
	}
	...
	public function set_elementor_flag(){
		update_option( 'musico_had_elementor', 'no' );
	}
	// Elementor dsiable default style
	public function elementor_desiable_default_style(){

		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'musico-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'musico_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style(){
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'musico-elementor-default-style',  MUSICO_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->musico_version );
		}
	}
	// Enqueue elementor notice scripts
	public function enqueue_elementor_notice_script(){

		$had_elementor = get_option( 'musico_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ){
			wp_enqueue_script( 'musico-elementor-notice', MUSICO_DIR_JS_URI.'musico-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'musico-elementor-notice',
				'musicoElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'musico-elementor-notice-nonce' ),
				)
			);
		}
