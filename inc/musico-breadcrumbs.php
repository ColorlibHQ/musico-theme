...
if ( ! function_exists( 'musico_breadcrumbs' ) ) {
    function musico_breadcrumbs( $args = array() ) {
        if ( is_front_page() ) {
            return;
        }
        ...
        $defaults  = array(
            ...
            'home_title'          => esc_html__( 'Home', 'musico' )
        );
        $args      = apply_filters( 'musico_breadcrumbs_args', wp_parse_args( $args, $defaults ) );
        ...
        */
        ...
        $html .= '<li class="breadcrumb-item"><a class="bread-link s-text16 bread-home" href="' . esc_url( get_home_url('/') ) . '" title="' . esc_attr( $args['home_title'] ) . '">'.esc_html__( 'Home', 'musico' ).' </a></li>';
        // Post
        if ( is_singular( 'post' ) ) {
            ...
            $html .= '<li class="breadcrumb-item active" aria-current="page">'.esc_html__('Search results for:','musico') . get_search_query() . '</li>';
        } elseif ( is_404() ) {
            $html .= '<li class="breadcrumb-item active" aria-current="page">' . esc_html__( 'Error 404', 'musico' ) . '</li>';
        } elseif ( is_home() ) {
            $html .= '<li class="breadcrumb-item active" aria-current="page">' . get_the_title( get_option( 'page_for_posts' ) ) . '</li>';
        }
        $html .= '</ol>';
        $html .= '</nav>';
        $html = apply_filters( 'musico_breadcrumbs_filter', $html );
