...
// Rating function
function musico_rating_function( $rating = 5 ) {
	$html = '';
	for( $i = 0; $i < $rating; $i++ ) {
...
// Tags function
function musico_get_tax_function( $taxonomy = 'musico-tag' ) {
...
function musico_meta( $id = '', $value_type = true ){
    
    $value = get_post_meta( get_the_ID(), '_musico_'.$id, $value_type );
    
    return $value;
}

// musico
function musico_get_styles_from_meta( $footer_bg_opt ) {
	$bg_styles = '';
...
// Blog Date Permalink
function musico_blog_date_permalink(){
...
// Blog Excerpt Length
if ( ! function_exists( 'musico_excerpt_length' ) ) {
	function musico_excerpt_length( $limit = 30 ) {
...
if ( ! function_exists( 'musico_posted_comments' ) ) :
    function musico_posted_comments(){
        
        $comments_num = get_comments_number();
        if( comments_open() ){
            if( $comments_num == 0 ){
                $comments = esc_html__('No Comments','musico');
            } elseif ( $comments_num > 1 ){
                $comments= $comments_num . esc_html__(' Comments','musico');
            } else {
                $comments = esc_html__( '1 Comment','musico' );
            }
            $comments = '<a href="' . esc_url( get_comments_link() ) . '">'. $comments .'</a>';
        } else {
            $comments = esc_html__( 'Comments are closed', 'musico' );
        }
...
function musico_iframe_match(){   
    $audio_content = musico_embedded_media( array('audio', 'iframe') );
    $iframe_match = preg_match("/\iframe\b/i",$audio_content, $match);
    return $iframe_match;
}

//Post embedded media
function musico_embedded_media( $type = array() ){
...
// WP post link pages
function musico_link_pages(){
    
    wp_link_pages( array(
    'before'      => '<div class="page-links"><span class="page-links-title">' . esc_html__( 'Pages:', 'musico' ) . '</span>',
    'after'       => '</div>',
    'link_before' => '<span>',
    'link_after'  => '</span>',
    'pagelink'    => '<span class="screen-reader-text">' . esc_html__( 'Page', 'musico' ) . ' </span>%',
    'separator'   => '<span class="screen-reader-text">, </span>',
    ) );
}

// Blog social sharing buttons
function musico_social_sharing_buttons( $ulClass = '', $tagLine = '', $show_label = false ) {
...
if( ! function_exists('musico_blog_single_post_navigation') ) {
	function musico_blog_single_post_navigation() {
				...
				<div class="row">
					<div class="col-lg-6 col-md-6 col-12 nav-left flex-row d-flex justify-content-start align-items-center">
						...
						if( get_next_post_link() ){
							...
							if( has_post_thumbnail() ){
								...
								<div class="thumb">
									<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
										<?php echo get_the_post_thumbnail( absint( $nextPost->ID ), 'musico_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
									</a>
								</div>
								<?php
							} ?>
							...
							<div class="detials">
								<p><?php echo esc_html__( 'Prev Post', 'musico' ); ?></p>
								<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
					...
					<div class="col-lg-6 col-md-6 col-12 nav-right flex-row d-flex justify-content-end align-items-center">
						...
						if( get_previous_post_link() ){
							...
							?>
							<div class="detials">
								<p><?php echo esc_html__( 'Next Post', 'musico' ); ?></p>
							...
							<div class="thumb">
								<a href="<?php the_permalink(  absint( $prevPost->ID )  ) ?>">
									<?php echo get_the_post_thumbnail( absint( $prevPost->ID ), 'musico_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
								</a>
							</div>
							<?php
						} ?>
...
// Musico Comment Template Callback
function musico_comment_callback( $comment, $args, $depth ) {
				...
				<div class="desc">
					...
					<div class="d-flex justify-content-between">
						<div class="d-flex align-items-center">
							<h5 class="comment_author"><?php printf( __( '<span class="comment-author-name">%s</span> ', 'musico' ), get_comment_author_link() ); ?></h5>
							<p class="date"><?php printf( __('%1$s at %2$s', 'musico'), get_comment_date(),  get_comment_time() ); ?><?php edit_comment_link( esc_html__( '(Edit)', 'musico' ), '  ', '' ); ?> </p>
							<?php if ( $comment->comment_approved == '0' ) : ?>
								<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'musico' ); ?></em>
								<br>
							<?php endif; ?>
						</div>
...
add_filter('comment_reply_link', 'musico_replace_reply_link_class');
function musico_replace_reply_link_class( $class ) {
...
// theme logo
function musico_theme_logo( $class = '' ) {

    $siteUrl = home_url('/');
    // site logo
		
	$custom_logo_id = get_theme_mod( 'custom_logo' );
	$imageUrl = wp_get_attachment_image_src( $custom_logo_id , 'musico_site_logo_80x80' );
	
	if( !empty( $imageUrl[0] ) ){
		$siteLogo = '<a class="'.esc_attr( $class ).'" href="'.esc_url( $siteUrl ).'"><img src="'.esc_url( $imageUrl[0] ).'" alt="'.esc_attr( musico_image_alt( $imageUrl[0] ) ).'"></a>';
	}else{
		$siteLogo = '<a class="'.esc_attr( $class ).'" href="'.esc_url( $siteUrl ).'"><img src="'.esc_url( MUSICO_DIR_IMGS_URI . 'logo.png' ).'" alt="'.esc_attr( get_bloginfo('name') ).'"></a>';
	}
...
function musico_pull_right( $id = '', $condation ){
...
function musico_get_textareahtml_output( $content ) {
...
function musico_get_slider_shortcode_options( $field ) {
...
// html Style tag
function musico_inline_bg_img( $bgUrl ){
    $bg = '';
...
function musico_sidebar_opt(){

    $sidebarOpt = musico_opt( 'musico-blog-sidebar-settings' );
    $sidebar = '1';
...
function musico_blog_pagination(){
	echo '<nav class="blog-pagination justify-content-center d-flex">';
        the_posts_pagination(
            array(
                'mid_size'  => 2,
                'prev_text' => __( '<span class="ti-angle-left"></span>', 'musico' ),
                'next_text' => __( '<span class="ti-angle-right"></span>', 'musico' ),
                'screen_reader_text' => ' '
            )
        );
	echo '</nav>';
...
// Themify Icon
function musico_themify_icon(){
    return [
        'seo_1' 	=> __('Search Icon', 'musico'),
        'seo_2' 	=> __('Mike Icon', 'musico'),
        'seo_3' 	=> __('Share Icon', 'musico'),
    ];
}

// CTA section
function musico_get_cta_area() {
	$cta_section_title      = !empty( musico_opt( 'musico_cta_section_title') ) ? musico_opt( 'musico_cta_section_title') : '';
	$cta_section_text       = !empty( musico_opt( 'musico_cta_section_text') ) ? musico_opt( 'musico_cta_section_text') : '';
	$cta_section_btn_label  = !empty( musico_opt( 'musico_cta_section_btn_label') ) ? musico_opt( 'musico_cta_section_btn_label') : '';
	$cta_section_btn_url    = !empty( musico_opt( 'musico_cta_section_btn_url') ) ? musico_opt( 'musico_cta_section_btn_url') : '';
	?>
    <!-- cta-area-start -->
...
function musico_get_contact_details_area() {
	?>
    <!-- location-area-start -->
    <div class="addres-area black-bg section-padding">
        <div class="container">
            <div class="row">
                <?php
				// Contact items
				$contact_items = musico_opt('musico_contact_details_section_fields');
                if ( $contact_items == true ) {
                    ...
                    if( is_array( $contact_items ) && count( $contact_items ) > 0 ){
                        foreach ($contact_items as $value) {
                            switch ($counter) {
                                case $counter == 2:
                                    $icon_img = MUSICO_DIR_ICON_IMG_URI . 'clock.png';
                                    break;
                                case $counter == 3:
                                    $icon_img = MUSICO_DIR_ICON_IMG_URI . 'envelope.png';
                                    break;
                                default:
                                    $icon_img = MUSICO_DIR_ICON_IMG_URI . 'globe.png';
                                    break;
                            }
                            $counter++;
