...
function musico_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'musico' ) . '</span></li>';
...
// Blog categories
function musico_featured_post_cat(){
...
// social media
if ( ! function_exists( 'musico_social' ) ) {
	function musico_social( $args = array() ) {
		...
		$url = musico_opt( 'musico_social_url' );
...
// header cart count
function musico_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( MUSICO_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'musico' ); ?>">
...
function musico_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'musico_contact7_form_content', 10, 2 );