...
function musico_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'musico' ),
        'next_string'     => esc_html__( '&raquo;', 'musico' ),
        'before_output'   => '<div class="musico-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'musico_pagination_defaults', $defaults )
    );
